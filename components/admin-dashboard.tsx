...
export default function AdminDashboard() {
  ...
  const saveChanges = () => {
    const countryClicks = countries.reduce(
      (acc, { country, clicks }) => {
        acc[country] = clicks
        return acc
      },
  ...
  const addNewCountry = () => {
    ...
    if (countries.some((c) => c.country.toLowerCase() === newCountry.toLowerCase())) {
      toast({
        title: "Error",
    ...
    setCountries((prev) => [...prev, { country: newCountry, clicks: clicksValue, isEditing: false }])
    ...
    toast({
      title: "Country Added",
      description: `Added ${newCountry} with ${clicksValue} clicks`,
    })
  }

  return (
    <div className="space-y-4">
      <h2 className="text-xl font-bold text-white">Manage Country Clicks</h2>
